...
public:
    ...
    RayTriangleIntersection intersect(Ray const & ray) const {
        ...
        RayTriangleIntersection currentIntersection;
        RayTriangleIntersection closestIntersection;

        for(unsigned int i = 0 ; i < this->triangles.size(); i++) {

            currentTriangle = Triangle(this->vertices[this->triangles[i].v[0]].position,this->vertices[this->triangles[i].v[1]].position,this->vertices[this->triangles[i].v[2]].position);

            currentIntersection = currentTriangle.getIntersection(ray); 
